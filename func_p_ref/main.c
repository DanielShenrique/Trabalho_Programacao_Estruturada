...
*/
int main(){//aqui temos o tipo e nome da variavel, ela não tem nenhum parâmetro, as "{" servem para dizer que esse é o começo da função
    ...
    printf("valor de y apos a funcao: %i\n", y);//aqui eu imprimo o valor de y, que não foi alterado, porque seu parâmetro foi dado por valor
...
Porque ele está sendo passado por referência, no lugar do parâmetro do int *a (que é passado por referência) quando a função for chamada no int main
Os demais não serão alterados porque estão sendo passados por valor 
*/
int MostrarValoresPorReferencia(int *a, int b, int c){//nessa linha se tem o tipo e nome da função, bem como seus parâmetros (o parâmetro int *a é passado por referência, os demais são passados por valor), o "{" significa o começo da função
    *a = *a + b + c; //aqui eu faço uma operação de soma de 3 valores alterando o a, que por ser por referência, alterará também a variavel que for passa da no lugar do a na referência da função.
    b = *a + c + b; //aqui eu faço uma operação de soma de 3 valores alterando o b, que não alterará a valor da variavel que será passada no parâmetro de b
    c = *a + c + b; //aqui eu faço uma operação de soma de 3 valores alterando o c, que não alterará a valor da variavel que será passada no parâmetro de c
